    ...
    public static boolean interceptMMCEProvideCrafting(Object provider, Object craftingTracker) {
        ...
        try {
            ...
            for (int i = 0; i < slots; i++) {
                ...
                if (!main.isWildcardPattern()) {
                    // Non-wildcard patterns are handled by MMCE original provideCrafting path.
                    continue;
                }
                List<SmartPatternDetails> candidates = main.expandToVirtualPatterns();
            ...
            if (injectedAny) {
                mmceDebug("interceptMMCEProvideCrafting: injected wildcard virtual patterns; continue MMCE original registration");
            }
            // Never short-circuit MMCE provideCrafting; otherwise other pattern items in the same
            // provider are skipped once an ItemTest pattern exists.
            return false;
        } catch (Throwable t) {
