...
public class TileEntityPatternExpander extends TileEntity implements IGridHost, ICraftingProvider, ITickable {
    ...
    private void createGridNode() {
        if (gridNode == null) {
            try {
                ...
                System.err.println("[PatternExpander] 閸掓稑缂撶純鎴炵壐閼哄倻鍋ｆ径杈Е: " + e.getMessage());
                e.printStackTrace();
            }
        }
    }
    ...
    private void scanAndExpandPatterns() {
        ...
        try {
            debugLog("Start scanning patterns");
            ...
            debugLog("闁规鍋呭璺ㄢ偓鐟版湰閸? expanded=" + currentCount + ", interfaces=" + interfacePatternMap.size()
            ...
            System.err.println("[PatternExpander] 闁规鍋呭鎸庡緞鏉堫偉袝: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void expandPatternsFromInterface(Object pushTarget, DualityInterface duality) {
        try {
            ...
            for (int i = 0; i < patterns.getSlots(); i++) {
                ...
                if (!(patternStack.getItem() instanceof ItemTest)) {
                    if (logSlotDetails) {
                        debugLog("AE2FC slot " + i + " 闂堢偞娅ら懗鑺ョ壉锟? item=" + patternStack.getItem().getClass().getName());
                    }
                    continue;
                }
                ...
                if (!ItemTest.hasEncodedItemStatic(patternStack)) {
                    if (logSlotDetails) {
                        debugLog("AE2FC slot " + i + " 闁哄牜浜炵槐顏堟儘? item=" + patternStack.getItem().getClass().getName());
                    }
                    continue;
                }
            ...
            debugLog("闁规亽鍎辫ぐ娑氫沪閺囩偟纾? host=" + pushTarget.getClass().getName() + ", slots=" + patterns.getSlots() + ", added=" + added);

        } catch (Exception e) {
            System.err.println("[PatternExpander] 閻忕偞娲栫槐鎴﹀冀闁垮绶插鎯扮簿鐟? " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void provideCrafting(ICraftingProviderHelper craftingTracker) {
        ...
        if (lastProvideCount != expandedPatterns.size()) {
            ...
            debugLog("闁圭粯鍔掔欢鐢稿冀闁垮绶查柡浣峰嵆閸? " + lastProvideCount);
        }
    }
    ...
    public boolean pushPattern(ICraftingPatternDetails patternDetails, InventoryCrafting table) {
        ...
        for (Map.Entry<Object, List<ICraftingPatternDetails>> entry : interfacePatternMap.entrySet()) {
            if (entry.getValue().contains(patternDetails)) {
                ...
                try {
                    ...
                    System.err.println("[PatternExpander] 闁规亽鍔戦埀顑跨劍閻楅亶寮堕崹顔轰杭閻? " + e.getMessage());
                    e.printStackTrace();
                    return false;
                }
            }
        ...
        System.err.println("[PatternExpander] 闁哄牜浜濇竟姗€宕氶悧鍫㈠闁哄鐏濋顔芥償閺冣偓鐢挳宕? " + patternDetails.getPattern());
    ...
    private ICraftingPatternDetails resolveAe2fcPattern(ICraftingPatternDetails patternDetails) {
        try {
            ...
            System.err.println("[PatternExpander] AE2FC 婵炵繝妞掔紞瀣冀闁垮绶查弶鐑嗗墯瀹曞弶寰勬潏顐バ? " + e.getMessage());
        }
    ...
    private ICraftingPatternDetails resolveAe2fcPatternFromStack(ItemStack patternStack) {
        ...
        try {
            ...
            System.err.println("[PatternExpander] AE2FC 濞翠椒缍嬮弽閿嬫緲閸掓稑缂撴径杈Е: " + e.getMessage());
        }
        ...
        try {
            ...
            System.err.println("[PatternExpander] AE2FC 闁哄瀚伴埀顒傚У缁侊附鎷呴幘瀵稿闁哄鐏濋妵鎴犳嫻? " + e.getMessage());
        }
        return null;
    }
    ...
    private ICraftingPatternDetails buildAe2fcFluidPatternDetails(SmartPatternDetails detail) {
        try {
            ...
            System.err.println("[PatternExpander] AE2FC FluidPatternDetails 閸掓稑缂撴径杈Е: " + e.getMessage());
            return null;
        }
    }
