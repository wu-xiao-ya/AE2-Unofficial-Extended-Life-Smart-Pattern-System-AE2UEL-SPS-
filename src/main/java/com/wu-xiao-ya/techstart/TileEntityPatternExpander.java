...
public class TileEntityPatternExpander extends TileEntity implements IGridHost, ICraftingProvider, ITickable {
    ...
    public void update() {
        if (world.isRemote) {
            return; // EN: Original comment text was corrupted by encoding.
            // ZH: 原注释因编码问题已损坏。
        }
    ...
    private void scanAndExpandPatterns() {
        ...
        try {
            ...
            debugLog("閹殿偅寮跨€瑰本鍨? expanded=" + currentCount + ", interfaces=" + interfacePatternMap.size()
            ...
            System.err.println("[PatternExpander] 閹殿偅寮挎径杈Е: " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void expandPatternsFromInterface(Object pushTarget, DualityInterface duality) {
        try {
            ...
            for (int i = 0; i < patterns.getSlots(); i++) {
                ...
                if (!(patternStack.getItem() instanceof ItemTest)) {
                    if (logSlotDetails) {
                        debugLog("AE2FC slot " + i + " 闈炴櫤鑳芥牱�? item=" + patternStack.getItem().getClass().getName());
                    }
                    continue;
                }
                ...
                if (!ItemTest.hasEncodedItemStatic(patternStack)) {
                    if (logSlotDetails) {
                        debugLog("AE2FC slot " + i + " 閺堫亞绱惍? item=" + patternStack.getItem().getClass().getName());
                    }
                    continue;
                }
            ...
            debugLog("閹恒儱褰涚仦鏇炵磻: host=" + pushTarget.getClass().getName() + ", slots=" + patterns.getSlots() + ", added=" + added);

        } catch (Exception e) {
            System.err.println("[PatternExpander] 鐏炴洖绱戦弽閿嬫緲婢惰精瑙? " + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    public void provideCrafting(ICraftingProviderHelper craftingTracker) {
        ...
        if (lastProvideCount != expandedPatterns.size()) {
            ...
            debugLog("閹绘劒绶甸弽閿嬫緲閺佷即鍣? " + lastProvideCount);
        }
    }
    ...
    public boolean pushPattern(ICraftingPatternDetails patternDetails, InventoryCrafting table) {
        ...
        for (Map.Entry<Object, List<ICraftingPatternDetails>> entry : interfacePatternMap.entrySet()) {
            if (entry.getValue().contains(patternDetails)) {
                ...
                try {
                    ...
                    System.err.println("[PatternExpander] 閹恒劑鈧焦鐗遍弶鍨亼鐠? " + e.getMessage());
                    e.printStackTrace();
                    return false;
                }
            }
        ...
        System.err.println("[PatternExpander] 閺堫亝澹橀崚鐗堢壉閺夊灝顕惔鏃€甯撮崣? " + patternDetails.getPattern());
    ...
    private ICraftingPatternDetails resolveAe2fcPattern(ICraftingPatternDetails patternDetails) {
        try {
            ...
            System.err.println("[PatternExpander] AE2FC 濞翠椒缍嬮弽閿嬫緲鏉烆剚宕叉径杈Е: " + e.getMessage());
        }
    ...
    private ICraftingPatternDetails resolveAe2fcPatternFromStack(ItemStack patternStack) {
        ...
        try {
            ...
            System.err.println("[PatternExpander] AE2FC 閺嬪嫰鈧姵绁︽担鎾寸壉閺夊灝銇戠拹? " + e.getMessage());
        }
        return null;
    }
    ...
    private static class GridBlockInfo implements IGridBlock {
        ...
        public double getIdlePowerUsage() {
            return 1.0; // EN: Original comment text was corrupted by encoding.
            // ZH: 原注释因编码问题已损坏。
        }
