...
public class ContainerPatternEditor extends Container {
    ...
    private int selectedRecipeType = 0; // EN: Original comment text was corrupted by encoding.
    // ZH: 原注释因编码问题已损坏。
    ...
    private ItemStack createGasMarkerStack(String gasName, int amount) {
        ...
        NBTTagCompound tag = marker.hasTagCompound() ? marker.getTagCompound() : new NBTTagCompound();
        tag.setBoolean("TechStartGasMarker", true);
        tag.setString("TechStartGasName", gasName);
        tag.setInteger("TechStartGasAmount", resolvedAmount);
        marker.setTagCompound(tag);
        return marker;
    }
    ...
    public void encodeSmartPattern() {
        ...
        String displayName = inputStacks.get(0).getDisplayName() + " 鈫?" + outputStacks.get(0).getDisplayName();
