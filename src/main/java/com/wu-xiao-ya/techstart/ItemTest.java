...
public class ItemTest extends Item implements ICraftingPatternItem {
     ...
     */
    public ItemTest() {
        setTranslationKey("sampleintegration.pattern_integrations");// EN: Temporary fallback comment after encoding recovery.
        // ZH: 编码修复后使用的临时兜底注释。
        setRegistryName("sampleintegration", "pattern_integrations");// EN: Original comment text was corrupted by encoding.
        // ZH: 原注释因编码问题已损坏。
        setCreativeTab(CreativeTabs.TOOLS);// EN: Original comment text was corrupted by encoding.
        // ZH: 原注释因编码问题已损坏。
        setMaxStackSize(1); // EN: Original comment text was corrupted by encoding.
        // ZH: 原注释因编码问题已损坏。
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if (player.isSneaking()) {
            ...
            if (!world.isRemote) {
                player.sendMessage(new TextComponentString("Pattern data cleared."));
            }
    ...
    public String getOreName(ItemStack stack) {
        ...
        if (!inputOreName.isEmpty()) {
            ...
            return inputOreName + " 鈫?" + outputOreName;
        }
    ...
    public void addInformation(ItemStack stack, World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        if (hasEncodedItem(stack)) {
            tooltip.add("搂a宸叉爣璁? 搂f" + getEncodedItemName(stack));
            ...
            List<Integer> outputGasAmounts = getOutputGasAmounts(stack);
                ...
                for (int i = 0; i < inputOres.size(); i++) {
                    int count = i < inputCounts.size() ? inputCounts.get(i) : 1;
                    tooltip.add("搂7杈撳叆" + (i + 1) + ": 搂f" + inputOres.get(i) + " 搂7x搂f" + count);
                }
                // EN: Temporary fallback comment after encoding recovery.
                // ZH: 编码修复后使用的临时兜底注释。
                for (int i = 0; i < inputFluids.size(); i++) {
                ...
                for (int i = 0; i < inputGases.size(); i++) {
                    int amount = i < inputGasAmounts.size() ? inputGasAmounts.get(i) : 0;
                    tooltip.add("§7Gas In " + (i + 1) + ": §f" + inputGases.get(i) + " §7x§f" + amount + "mB");
                }
                ...
                for (int i = 0; i < outputOres.size(); i++) {
                    int count = i < outputCounts.size() ? outputCounts.get(i) : 1;
                    tooltip.add("§7Output " + (i + 1) + ": §f" + outputOres.get(i) + " §7x§f" + count);
                }
                // EN: Temporary fallback comment after encoding recovery.
                // ZH: 编码修复后使用的临时兜底注释。
                for (int i = 0; i < outputFluids.size(); i++) {
                ...
                for (int i = 0; i < outputGases.size(); i++) {
                    int amount = i < outputGasAmounts.size() ? outputGasAmounts.get(i) : 0;
                    tooltip.add("§7Gas Out " + (i + 1) + ": §f" + outputGases.get(i) + " §7x§f" + amount + "mB");
                }
            } else {
                tooltip.add("§7Ore Type: §f" + getOreName(stack));
            }
            
            tooltip.add("§7Right-click to open editor");
        } else {
            tooltip.add("§eRight-click to open pattern editor");
            tooltip.add("§7Put ore dictionary items, fluid or gas containers in the GUI to mark.");
        }
    }
